...
APP_ID="6760979268"   # ExifArmor
TRAIN=$(/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" \
  "$CI_PRIMARY_REPOSITORY_PATH/ExifArmor/Info.plist" 2>/dev/null \
...
if [ -z "$TRAIN" ] || [ "$TRAIN" = "\$(MARKETING_VERSION)" ]; then
  TRAIN=$(grep -m1 "MARKETING_VERSION = " "$CI_PRIMARY_REPOSITORY_PATH/ExifArmor.xcodeproj/project.pbxproj" \
